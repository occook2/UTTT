# ttt/scripts/view_tournament.py
...
from ttt.env.state import TTTEnv, action_to_rc
...
class ReplayApp:
    def __init__(self, root: tk.Tk, payload: Dict[str, Any]):
        ...
        # ---- UI layout ----
        self.root.title("TTT Tournament Viewer")
    ...
    def _redraw(self):
        ...
        if last is not None:
            ...
            cx = x0 + lc * CELL
            cy = y0 + lr * CELL
            self.canvas.create_rectangle(cx + 3, cy + 3, cx + CELL - 3, cy + CELL - 3, outline="orange", width=HILITE_W)
...
def main():
    ...
    if not path:
        ...
        path = filedialog.askopenfilename(
            ...
            initialdir=os.path.join(os.getcwd(), "ttt", "tournaments") if os.path.isdir(os.path.join(os.getcwd(), "ttt", "tournaments")) else os.getcwd(),
        )
        _root.destroy()
    if not path:
