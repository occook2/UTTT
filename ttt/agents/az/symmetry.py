...
from ttt.agents.az.self_play import TrainingExample
...
def rotate_example(example: TrainingExample, k: int) -> TrainingExample:
    ...
    """
    # Rotate the board state (shape: 5, 3, 3) along the last two axes
    rotated_state = np.rot90(example.state, k=k, axes=(1, 2)).copy()
    
    # Rotate the policy (shape: 9) as a 3x3 grid, then flatten
    rotated_policy = np.rot90(example.policy.reshape(3, 3), k=k).flatten().copy()
...
def reflect_example(example: TrainingExample) -> TrainingExample:
    ...
    reflected_policy = np.flip(example.policy.reshape(3, 3), axis=1).flatten().copy()
    return TrainingExample(
